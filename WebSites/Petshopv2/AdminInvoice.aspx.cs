...
public partial class AdminInvoice : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        dt.Columns.Add("JobType", Type.GetType("System.String"));
        ...
        {
            dt.Rows.Add();
            dt.Rows[i]["QTY"] = (i + 1).ToString();
            dt.Rows[i]["JobType"] = "Item " + i.ToString();
            dt.Rows[i]["Price"] = (i + 1).ToString();
            dt.Rows[i]["Total"] = (i + 1).ToString();
            totalAmtStr += (i + 1);
        }

        //GridView 
        GRIDINVOICETEMP.DataSource = dt;
        GRIDINVOICETEMP.DataBind();
    }

    //class variables and gridview databind
    //Class Variables
    ...
    protected MemoryStream CreatePDF()
    {
        ...
        PdfPCell nextPostCell1 = new PdfPCell(new Phrase("PetShoppe", bodyFont));
        nextPostCell1.Border = Rectangle.LEFT_BORDER | Rectangle.RIGHT_BORDER;
        nested.AddCell(nextPostCell1);
        PdfPCell nextPostCell2 = new PdfPCell(new Phrase("20 Hobson Street,", bodyFont));
        nextPostCell2.Border = Rectangle.LEFT_BORDER | Rectangle.RIGHT_BORDER;
        nested.AddCell(nextPostCell2);
        PdfPCell nextPostCell3 = new PdfPCell(new Phrase("Auckland CBD", bodyFont));
        ...
        headertable.AddCell(new Phrase(Date, bodyFont));
        ...
        headertable.AddCell(new Phrase(TXTBXCUSTID.Text + "\n" + TXTBXCUSTNAME.Text, bodyFont));
        ...
        PdfPCell cell1 = new PdfPCell(new Phrase("QTY", boldTableFont));
        cell1.HorizontalAlignment = 1;
        itemTable.AddCell(cell1);
        PdfPCell cell2 = new PdfPCell(new Phrase("JobType", boldTableFont));
    ...
    protected void CHKCITRUS_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKCITRUS.Checked == true)
        {
            ...
            // for counting jobtype Citrus...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as CITRUSQTY from InvoiceTransaction where JobType = 'Citrus'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYCITRUS.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKFLEARELIEF_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKFLEARELIEF.Checked == true)
        {
            ...
            // for counting jobtype Flea relief ...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as FLEAQTY from InvoiceTransaction where JobType = 'FleaRelief'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYFLEA.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKKISSABLE_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKKISSABLE.Checked == true)
        {
            ...
            // for counting jobtype Kissable...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as KISSQTY from InvoiceTransaction where JobType = 'Kissable'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYKISS.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKDYE_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKDYE.Checked == true)
        {
            ...
            // for counting jobtype Dye...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as DYEQTY from InvoiceTransaction where JobType = 'Dye'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYDYE.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKCUT_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKCUT.Checked == true)
        {
            ...
            SqlConnection conn = new SqlConnection(connStr);
            conn.Open();
            str = "select Price from JobTypeTable where JobType = 'Cut' ";
            cmd = new SqlCommand(str, conn);
            SqlDataReader reader = cmd.ExecuteReader();
            ...
            // for counting jobtype Cut...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as CUTQTY from InvoiceTransaction where JobType = 'Cut'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYCUT.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKNAILTRIM_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKNAILTRIM.Checked == true)
        {
            ...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as NTRIMQTY from InvoiceTransaction where JobType = 'NailTrim'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
    ...
    protected void CHKPEDICURE_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKPEDICURE.Checked == true)
        {
            ...
            // for counting jobtype Pedicure...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as PEDICUREQTY from InvoiceTransaction where JobType = 'Pedicure'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYPEDICURE.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKFACIAL_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKFACIAL.Checked == true)
        {
            ...
            // for counting jobtype Facial...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as FACIALQTY from InvoiceTransaction where JobType = 'Facial'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYFACIAL.Text = count.ToString();
                conn.Close();
            }
    ...
    protected void CHKSHEDDING_CheckedChanged(object sender, EventArgs e)
    {
        if (CHKSHEDDING.Checked == true)
        {
            ...
            // for counting jobtype Shedding...
            SqlCommand cmd2 = new SqlCommand("select Count(JobType) as SHEDDINGQTY from InvoiceTransaction where JobType = 'Shedding'", conn);
            object count = cmd2.ExecuteScalar();
            if (count != null)
            {
                TXTBXQTYSHEDDING.Text = count.ToString();
                conn.Close();
            }
